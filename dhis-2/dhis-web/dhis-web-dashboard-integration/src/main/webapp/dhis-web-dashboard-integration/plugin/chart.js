            ...
            api.layout.Layout = function(config, applyConfig, forceApplyConfig) {
				...
				validateSpecialCases = function() {
					...
					if (objectNameDimensionMap[dimConf.operand.objectName] && objectNameDimensionMap[dimConf.indicator.objectName]) {
						web.message.alert('Indicators and detailed data elements cannot be specified together.', true);
						return;
					}

					// dc and de
					if (objectNameDimensionMap[dimConf.operand.objectName] && objectNameDimensionMap[dimConf.dataElement.objectName]) {
						web.message.alert('Detailed data elements and totals cannot be specified together.', true);
						return;
					}

					// dc and ds
					if (objectNameDimensionMap[dimConf.operand.objectName] && objectNameDimensionMap[dimConf.dataSet.objectName]) {
						web.message.alert('Data sets and detailed data elements cannot be specified together.', true);
						return;
					}

					// dc and co
					if (objectNameDimensionMap[dimConf.operand.objectName] && objectNameDimensionMap[dimConf.category.objectName]) {
						web.message.alert('Categories and detailed data elements cannot be specified together.', true);
						return;
					}

                    // in and aggregation type
                    if (objectNameDimensionMap[dimConf.indicator.objectName] && config.aggregationType !== 'DEFAULT') {
                        web.message.alert('Indicators and aggregation types cannot be specified together.', true);
                        return;
                    }
                ...
                return function() {
					...
					if (!(config && Ext.isObject(config))) {
						web.message.alert('Layout: config is not an object (' + init.el + ')', true);
						return;
					}
					...
					if (!config.columns) {
						web.message.alert('No series items selected');
						return;
					}

					if (!config.rows) {
						web.message.alert('No category items selected');
						return;
					}
					...
					if (!Ext.Array.contains(objectNames, dimConf.period.objectName)) {
						web.message.alert('At least one period must be specified as series, category or filter');
						return;
					}
