...
|:---|:---|:---|:---|
| [Proxy Recording](#method-1-proxy-recording) | When you can configure browser proxy settings | Direct capture with powerful filtering, no file handling | Requires proxy configuration |
| [HAR Import](#method-2-har-import) | When you cannot configure proxy (e.g., restricted permissions) | No proxy setup, works with any browser | Extra export step, browser network filtering less powerful than Gatling's filters |

## Method 1: Proxy Recording

### Step 1: Start Gatling Recorder

In `dhis-2/dhis-test-performance`:
...
Click **Stop & Save** in the Recorder GUI. The Gatling simulation is generated at:
...
1. **IMPORTANT**: Uncheck "Save preferences" at the bottom (workaround for Gatling bug)
2. Select "HAR Converter" mode from dropdown (top right)
3. Click folder icon to browse and select your `traffic.har` file
...
> [!NOTE]
> The HAR converter does not respect the `package` or `className` settings from
...
> [!IMPORTANT]
> Simulations require authentication. Choose one:
>
...
> [!WARNING]
> **Known Issue**: Gatling 4.x has a bug where request body files (`*_request.json`) are placed at
> the root of `src/test/resources/` instead of in the package subdirectory where the generated code
...
> [!IMPORTANT]
> Change the base URL if you recorded against a production or remote server and want to replay
...
> [!NOTE]
> If you enabled "Use HTTP method and URI as request postfix" in the recorder settings, the
...
* **Add load injection**: Replace `atOnceUsers(1)` with a profile that matches your implementation.
...
* **Clean up headers**: Remove browser-specific headers and ETags
* **Add assertions**: `global().successfulRequests().percent().gte(99.0)`
* **Use groups**: Organize related requests with `group("Search TEs").on(...)`
...
Edit `src/test/resources/recorder.conf` to customize port, output location, or filters. See
`mvn gatling:help -Ddetail=true -Dgoal=recorder` for all options.
...
* **Tests fail with 401 Unauthorized**: Recording doesn't include login. Add authentication to the
