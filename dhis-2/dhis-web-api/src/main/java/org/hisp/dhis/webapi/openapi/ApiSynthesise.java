 ...
 * <p>
 * After the {@link Api} has been analysed by looking at the controller endpoints in the first step
 * the gathered information is completed and consolidated in this second step. This is the preperation to be ready to transform the {@link Api} into the OpenAPI document in the third step.
 *
 * @author Jan Bernitt
 */
@Value
...
public class ApiSynthesise
{
    ...
    private void nameSharedSchemas() {
        ...
        api.getSchemas().entrySet().stream()
            ...
            .forEach( e -> sharedSchemasByName.computeIfAbsent(
                e.getValue().getSharedName().orElse( e.getKey().getSimpleName() ), name -> new ArrayList<>() ).add( e ) );
    ...
    /**
     * Traverses the {@link Api} document for all {@link org.hisp.dhis.webapi.openapi.Api.Schema}s to collect the additional schemas.
     */
    private void nameSharedAdditionalSchemas() {
        ...
        addAdditionalSchemas( to,
            api.getComponents().getParameters().values().stream().flatMap( List::stream ).map( Api.Parameter::getType ) );
    ...
    /**
     * Parameter types may use the same shared name as long as they do not have the same parameter name.
     * If so this is a clash.
     */
    private void checkNoParameterNameClash(Map<String, List<Map.Entry<Class<?>, List<Api.Parameter>>>> paramsBySharedName) {
    ...
    {
        return createNameClashMessage( name, schemas, "More than one parameter type uses the shared name `%s` and contains at least one parameter of the same name: \n\t- %s" );
    }
    ...
    private static String createNameClashMessage( String name, List<? extends Map.Entry<Class<?>, ?>> usages, String template )
    {
    ...
    private String getAdditionalTypeName(Api.Schema schema) {
        ...
        return
            prefix + config.getNamePartDelimiter() + api.getSchemas().get( of ).getSharedName().getValue();
    }
}
