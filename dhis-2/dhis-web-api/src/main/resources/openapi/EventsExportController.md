...
value. Special characters like `+` need to be percent-encoded so `%2B` instead of `+`. 
...
like `filter=AuPLng5hLbE:gt:438901703:lt:448901704` are allowed. Repeating the same attribute UID is not allowed. 
Operator and values are case-insensitive. A user needs metadata read access to the attribute and data read access 
to the program (if the program is without registration) or to the program stage (if the program is with registration).
