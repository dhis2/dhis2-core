...
mvn clean install --threads 2C -DskipTests -Dmaven.test.skip=true -f "${DIR}/pom.xml" -pl -dhis-web-embedded-jetty,-dhis-test-integration,-dhis-test-coverage,-dhis-web-tomcat
mvn clean install -PjibDockerBuild -DactiveProfile=jibDockerBuild -DskipTests -Dmaven.test.skip=true -f "${DIR}/dhis-web-portal/pom.xml" $JIB_PROFILE \
