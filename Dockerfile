...
RUN mvn clean install --batch-mode --no-transfer-progress -f dhis-2/pom.xml -DskipTests -pl -dhis-web-embedded-jetty
RUN mvn clean install --batch-mode --no-transfer-progress -U -f dhis-2/dhis-web/pom.xml -DskipTests
